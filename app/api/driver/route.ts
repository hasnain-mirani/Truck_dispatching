...
export async function POST(_request: NextRequest) {
  try {
    const body = (await _request.json()) as { name: string; licenseNumber: string }
    ...
    const response = await apiClient.post("/driver", { name, licenseNumber })
...
export async function GET() {
  try {
    ...
    const data = response.data as {
      success: boolean
      message: string
      responseObject: { id: string; name: string; licenseNumber: string }[]
      statusCode: number
    }
